...
def crossing(wire1,wire2):
    route1 = coords(wire1)
    route2 = coords(wire2)
    crossings = {}
    for coordinate in route1:
        if coordinate in route2:
            crossings[coordinate]=route1[coordinate]+route2[coordinate]
    key_min = min(crossings.keys(), key=(lambda k: crossings[k]))
    return crossings[key_min]
